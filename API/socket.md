...
Realtime communication with Party DJ happens through the WebSocket server. In particular, every communication that is
initiated by Party DJ happens through the socket server, as well as sending chat messages.

Party DJ sends information like users joining, chat messages coming in, and song advances over the socket API. As a
client, you can send authentication messages and chat messages.

## Connecting To The Socket Server

The socket server is a WebSocket server that lives at `wss://partydj.herokuapp.com`. The socket server is separated
from Party DJ's other servers, so you have to do a little bit of work to authenticate yourself to it.

The Party DJ socket is written in Go!
...
These examples will assume JavaScript and Node.js, but the principles apply to every language/platform/library.
 ...
 1. First, you need to connect to the server. This is the easiest part&em;just use your favourite WebSocket library:
    ...
    var socket = new WebSocket('wss://partydj.herokuapp.com', {
        origin: 'https://partydj.xyz'
    });
    ```
    As you may notice, though, there is a small caveat here: the socket server only accepts connections that originate
    from https://partydj.xyz. Browsers set the `Origin:` header automatically, but your websocket library may not! If you're
...
The `h` message is simply that, a packet containing a one character string: `h`. Party DJ occasionally sends these to make
...
chatting or the DJ cycle setting changing to a Party DJ maintenance announcement (the latter being the most common!
 ...
 * The `s` property is short for "slug". This tells you which community the event happened in. It contains the room slug
   (the community URL without `https://partydj.xyz/`), or "dashboard" for events that don't have a community. Of course, you
...
You can only send two types of messages to Party DJ: The "auth" and "chat" messages. They follow a format that's somewhat
...
messages, because you can only be in a single room at a time--so Party DJ already knows where your message should go.

The `t` property is new: It's the UNIX time at which you sent your message. (In seconds, so if you're using JavaScript
you should divide by 1000 first: `Math.floor(Date.now() / 1000)`.) Theoretically, the Party DJ web app offsets this to be
close to the Party DJ server time using the `window._st` JavaScript variable, but it doesn't appear to affect much. For
...
connection is set up. Party DJ won't wait for you for long, so you can't open the socket and send your message ten
seconds later.
...
Note that chat is rate limited, and Party DJ will silently drop messages if you send too many too quickly. If you add a
1-second interval between messages on your end, you should be safe. (This is what the PlugAPI library does.) Party DJ's
...
You cannot send newline characters (`\n`) in your messages, and Party DJ will again silently drop your messages if you
